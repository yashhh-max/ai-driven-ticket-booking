...
async function setShadowProtocolTimer() {
  try {
    console.log('⏱️  Setting Shadow Protocol auto-booking timer to 3 minutes...\n');

    // First, reset Varanasi to a far future date
    const { data: varanasi } = await supabase
      .from('movies')
      .select('id, title')
      .eq('title', 'Varanasi')
      .single();
    ...
    if (varanasi) {
      const { data: varansiShowtimes } = await supabase
        .from('showtimes')
        .select('id')
        .eq('movie_id', varanasi.id)
        .order('show_date', { ascending: true })
        .limit(1);

      if (varansiShowtimes && varansiShowtimes.length > 0) {
        ...
        await supabase
          ...
          .eq('id', varansiShowtimes[0].id);

        console.log('✅ Reset Varanasi movie release time to 30 days in future\n');
      }
    }

    // Now set Shadow Protocol to 3 minutes from now
    const { data: movie, error: movieError } = await supabase
      .from('movies')
      .select('id, title')
      .eq('title', 'Shadow Protocol')
      .single();

    if (movieError || !movie) {
      console.error('❌ Could not find Shadow Protocol movie');
      return;
    }
    ...
    console.log('✅ Successfully set Shadow Protocol ticket release time!\n');
    console.log(`📝 Test Instructions:`);
    console.log(`1. Go to Auto-Book page`);
    console.log(`2. Pre-book "Shadow Protocol" movie for ${showtime.show_date} at ${showtime.show_time}`);
