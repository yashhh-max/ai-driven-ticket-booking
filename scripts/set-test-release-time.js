...
async function setTestReleaseTime() {
  try {
    console.log('⏱️  Setting up test auto-booking with 3-minute release time...\n');

    // Get the first movie (Spirit)
    ...
    if (movieError || !movie) {
      console.error('❌ Could not find Spirit movie');
      return;
    }
    ...
    console.log('✅ Successfully set ticket release time!');
    console.log(`\n📝 Test Instructions:`);
    console.log(`1. Go to Auto-Book page`);
    console.log(`2. Pre-book "Spirit" movie for ${showtime.show_date} at ${showtime.show_time}`);
