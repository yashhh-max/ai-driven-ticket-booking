-- SIMPLE FIX: process_pre_booking function
...
BEGIN
  ...
  -- Check if all seats are available (don't select price - it doesn't exist)
  FOR v_seat IN 
    SELECT pbs.seat_id
  ...
  EXCEPTION WHEN OTHERS THEN
    v_error_msg := SQLERRM;
    UPDATE pre_bookings SET status = 'failed', failure_reason = v_error_msg, processed_at = NOW(), updated_at = NOW()
    WHERE id = p_pre_booking_id;
    RETURN json_build_object('success', false, 'error', v_error_msg);
  END;
  
  -- Create booked seats (NO PRICE COLUMN - it doesn't exist)
  BEGIN
  ...
  UPDATE pre_bookings 
  SET status = 'completed', booking_id = v_booking_id, processed_at = NOW(), updated_at = NOW()
  WHERE id = p_pre_booking_id;
  ...
  WHERE id = (
    ...
    WHERE wt.wallet_id = (SELECT w.id FROM wallets w WHERE w.user_id = v_pre_booking.user_id)
      AND wt.booking_id IS NULL AND wt.type = 'deduction'
